...
async function triggerSensor(sensorName: string) {
    try {
        const eventStartTime = Date.now();
        
        // Visual feedback
        const btnElement = document.querySelector(sensorButtonSelector[sensorName as keyof typeof sensorButtonSelector]);
        if (btnElement) {
        ...
        const priority = sensorPriority[sensorName as keyof typeof sensorPriority];
...
function animateCarForSensor(sensorName: string) {
...
function updateCarStatus(property: string, value: string, className: string) {
...
function addSystemEventLog(message: string, type: string) {
...
async function fetchAndDisplayEventLogs() {
    try {
        ...
        if (data.events && data.events.length > 0) {
            ...
            if (systemLog) {
                ...
                recentEvents.forEach((event: string) => {
...
function createDetailedLogEntry(eventText: string): HTMLElement | null {
    const entry = document.createElement('div');
...
function formatMicrosecondTimestamp(microsecondStr: string): string {
